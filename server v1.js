...
app.post('/api/bots/create', validateBotConfig, async (req, res) => {
    try {
        ...
        // En tu server.js, dentro de la ruta /api/bots/events/qr
        broadcastToClients({
            type: 'bot_created',
            botId: botConfig.id,
...
app.post('/api/bots/events/qr', async (req, res) => {
    console.log('[Server] Recibida solicitud QR. req.body:', req.body); // <-- AGREGA ESTO

    try {
        const { botId, qrCodeData, qrImagePath } = req.body; // Asegúrate de que el bot envía estos datos
        // Aquí puedes procesar el QR:
        // 1. Guardarlo en la base de datos para que la interfaz web lo consulte.
        // 2. Transmitirlo a los clientes conectados por WebSocket.

        console.log(`[Server] QR recibido para bot ${botId}: ${qrCodeData || 'N/A'}. Ruta de imagen: ${qrImagePath || 'N/A'}`);

        // Ejemplo: Guardar en la base de datos (asegúrate de que tu db.js tiene esta función)
        await db.updateBot(botId, { status: 'qr_scanned', qr: qrCodeData, qrImagePath: qrImagePath });
...
const QRCodeImage = require('qrcode'); // Asegúrate de instalar qrcode con npm install qrcode
const qrcode = require('qrcode-terminal');
const axios = require('axios');
const fs = require('fs-extra'); // fs-extra no es un módulo "core" de Node.js, necesita npm install
const express = require('express');
const cors = require ('cors');
const bodyParser = require=('body-parser');
let qrReported = false; // Declara esto fuera de la función de manejo del QR, a nivel global o del cliente
...
// **IMPORTANTE**: URL del servidor de gestión
const MANAGEMENT_SERVER_URL = process.env.MANAGEMENT_SERVER_URL || 'http://localhost:8080';
...
async function reportQr(qrCodeData, qrImagePath) {
    console.log(\`[Bot] Función reportQr llamada en \${new Date().toLocaleTimeString()}\`);
    // La URL se construye aquí usando la variable que el bot lee de sus variables de entorno
    const reportUrl = \`\${MANAGEMENT_SERVER_URL}/api/bots/events/qr\`; 
    console.log(\`[Bot] Intentando reportar QR a la URL: \${reportUrl}\`);
    try {
        ...
        console.log('[Bot] QR reportado exitosamente:', response.data);
    } catch (error) {
        console.error('[Bot] Error reportando evento qr (capturado):', error.message);
        if (error.response) {
            console.error('[Bot] Código de estado del error:', error.response.status);
            console.error('[Bot] Datos del error:', error.response.data);
        }
    }
}
...
client.on('qr', async qr => {
  console.log('[QR] Código QR recibido para bot:');
  qrcode.generate(qr, { small: true });

  try {
    // Generar imagen PNG en el directorio del bot
    const imagePath = path.resolve(__dirname, 'qr_'+BOT_ID+'.png');
...
client.on('authenticated', async () => {
    console.log('[Auth] bot autenticado exitosamente!');
    
    // **REPORTAR AUTENTICACIÓN**
    await reportEventToServer('authenticated', {
        botName: 'bot',
        timestamp: new Date().toISOString()
    });
});
...
client.on('ready', async () => {
    console.log('[Ready] bot está listo y operativo!');
    isBotOperational = true;
    
    // **REPORTAR QUE ESTÁ LISTO**
    await reportEventToServer('ready', {
        botName: 'bot',
        timestamp: new Date().toISOString()
    });
